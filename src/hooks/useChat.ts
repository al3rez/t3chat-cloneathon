...
export function useChat() {
  ...
  const createChatAndSendMessage = useCallback(
    async (content: string, useWebSearch: boolean = false) => {
      if (!user) {
        return;
      }
      ...
      try {
        ...
        try {
          ...
          if (useWebSearch && selectedModel.includes("gemini")) {
            aiResult = await generateAIResponseWithSearch(
              conversationHistory,
              selectedModel,
            );
          } else {
          ...
          setChats((prev) =>
            prev.map((chat) => (chat.id === thread.id ? finalChat : chat)),
          );
          setIsLoading(false);
          ...
          const errorMessage: Message = {
            id: (Date.now() + 1).toString(),
            content:
              "Sorry, I encountered an error while processing your request. Please check your API key settings.",
            role: "assistant",
            timestamp: new Date(),
            thread_id: thread.id,
          };
          ...
          setChats((prev) =>
            prev.map((chat) => (chat.id === thread.id ? errorChat : chat)),
          );
        }
      } catch (error) {
  ...
  const sendMessage = useCallback(
    async (content: string, useWebSearch: boolean = false) => {
      ...
      try {
        ...
        setActiveChat(errorChat);
        setChats((prev) =>
          prev.map((chat) => (chat.id === activeChat.id ? errorChat : chat)),
        );
      }
    },
